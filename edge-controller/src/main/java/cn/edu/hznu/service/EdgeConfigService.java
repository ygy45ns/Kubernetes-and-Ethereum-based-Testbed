...
public class EdgeConfigService {
    ...
    public void resetTaskRate() {
        for (int i = 0; i < edgeNodeNumber; i++) {
            var edgeNodeConfig = edgeNodeService.getOne(new QueryWrapper<EdgeNode>().eq("name", "edge-node-" + (i+1)));
            edgeNodeConfig.setTaskRate(taskRateRandom.sample());
            edgeNodeService.updateById(edgeNodeConfig);
        }
    }
}